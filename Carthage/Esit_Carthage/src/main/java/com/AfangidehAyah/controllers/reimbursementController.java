...
public class reimbursementController {
	...
	public Handler getReimbursementsAuthHandler = (ctx) -> {
		...
		if (AuthController.ses != null) { // if the user is logged in, they can access this functionality
			// int employee_id = Integer.parseInt(ctx.cookie("employeeid"));
			employees user = (employees) AuthController.ses.getAttribute("user");

			// We need an ArrayList of Employees, courtesy of our EmployeeDAO
			ArrayList<reimbursements> reimbursement = rDAO.getReimbursementsbyAuthor(user.getEmployee_id());
	...
	public Handler updatereimbursement = (ctx) -> {
		...
		// in this case, our Launcher endpoint handler calls it "title", so this is what
		// we need to call here

		// int to hold the new Role salary, which the user will include in the BODY of
		// the HTTP Request
		int reimb_status = Integer.parseInt(ctx.body()); // we need to use parseInt here, because ctx.body() returns a
																											// String
		// in postman, no need to make a JSON object, we can just input whatever number
		// we want.
